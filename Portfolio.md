...
**Scoring or feedback**:  
When a song is completed, the results page will pop up, showing the player's overall accuracy as a percentage and the number of notes rated as "Perfect", "Good", "Bad", or "Miss", allowing them to reflect on their performance and track improvement over time. During gameplay, the system will also provide real time feedback on each note's accuracy to maintain player engagement.

### **Non-Functional Requirements**:
**Performance requirements**:   
The system should run smoothly on the player's device with almost immediate response (1-2 seconds). Falling notes, audio and scoring calculations (should appear under 0.5 seconds after the note is pressed) must not lag and should occur in real time to ensure exactness.
...
Although my game currently focuses on a single chart, it may not be suitable for players of all skill levels and abilities. To improve accessibility, adjustable settings (mentioned prior) will allow players to customise their experience.
If possible, multiple difficulty levels, translation to different languages, and larger notes/ high contrast visuals could be added for inclusivity later on.
...
Content within the game, including visuals, text and symbols, will avoid harmful stereotypes and biases. The characters in the games are cats, which means the designs are neutral and unlikely to misrepresent any groups of players. 
